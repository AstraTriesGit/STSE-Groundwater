---
title: "Combining the Collected GWL Datasets"
output: pdf_document
---
...
- May 2018 - May 2023
- May 2014 - May 2018
- May 2004 - May 2014 (TO BE RECEIVED)
```{r}
library(readxl)
library(dplyr)
library(purrr)
...
We read all the Excel files provided by India-WRIS and combined them into different `.feather` files
depending on the period and the measurement type (manual/telemetric.)
```{r}
...
No NAs in data. However, (latitude, longitude) are strings. We have to convert them to doubles, but the
numbers are possibly too precise. Pending evaluation of null values.
```{r}
...
Invalid values have not been dealt with, and handling these are beyond the capabilities of standard cleaning procedures.
