...
```{r}
# now give me the stations who did something in May
may_stations <- gwl_d3_unclean %>%
  filter(data_time == as.Date('2023-05-01')) %>%
  group_by(latitude, longitude) %>%
  summarise(lol = n())

aug_stations <- gwl_d3_unclean %>%
...
may_aug_3 <- may_stations %>%
  inner_join(aug_stations, by = c('latitude', 'longitude'))
